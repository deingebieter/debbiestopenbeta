...
/**
 * VERBESSERTE Funktion zum Ein-/Ausblenden des Raten-Eingabefelds
 */
function toggleRateInput() {
  ...
  if (!rateInputSection) {
    console.error("Element 'rateInputSection' wurde nicht gefunden!");
    return;
  }
...
function saveDebtEntry() {
  ...
  const debtEntry = {
    ...
    paymentHistory: [],
    paymentStatus: {  // KORRIGIERT: Verwendung von paymentStatus statt status
      delay: false,
      defer: false,
      refusal: false
    }
...
// Stellen Sie sicher, dass die Ereignisverarbeitung beim Laden der Seite richtig initialisiert wird
document.addEventListener('DOMContentLoaded', function() {
  const installmentPlanSelect = document.getElementById('installmentPlan');
  if (installmentPlanSelect) {
    // Zuerst alles entfernen (zur Sicherheit)
    installmentPlanSelect.removeEventListener('change', toggleRateInput);
    
    // Dann neu hinzufügen
    installmentPlanSelect.addEventListener('change', toggleRateInput);
    
    // Auch im Markup direkt hinzufügen (zusätzliche Absicherung)
    installmentPlanSelect.setAttribute('onchange', 'toggleRateInput()');
    
    console.log("Event-Listener für installmentPlan wurde (neu) hinzugefügt");
    
    // Initial prüfen, ob das Feld angezeigt werden soll
    toggleRateInput();
  } else {
